...
def write_cache(key, value, timeout = None):
    ...
    if timeout is None:  RedisClient.set(key, value)
    else: RedisClient.setex(key, timeout, value)
...
def youtube_videos_tags(ids):
    ...
    for vid in ids:
        k = 'video.' + vid
        value = read_cache(k)
        if value: results[vid] = value['snippet'].get('tags', [])
        else: query_ids.append(vid)
