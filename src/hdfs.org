...
HDFS文件系统的操作步骤主要如下：
   ...
   - 代码可以参考 [[file:~/repo/dirtysalt.github.io/codes/java/hdfs/src/main/java/com/dirlt/java/hdfs/GetFS.java][code on github]]
...
** 一致性问题
  - hdfs一致性模型是reader不能够读取到当前被write的block，除非writer调用sync强制进行同步
    ...
    - #note: 但是似乎不太work. 参考代码 [[file:~/repo/dirtysalt.github.io/codes/java/hdfs/src/main/java/com/dirlt/java/hdfs/TestConsistency.java][code on github]]
...
** 读写进度
  ...
  - 代码可以参考 [[file:~/repo/dirtysalt.github.io/codes/java/hdfs/src/main/java/com/dirlt/java/hdfs/TestProgress.java][code on github]]

** 获取集群运行状况
  - 参考代码 [[file:~/repo/dirtysalt.github.io/codes/java/hdfs/src/main/java/com/dirlt/java/hdfs/ClusterSummary.java][code on github]]
