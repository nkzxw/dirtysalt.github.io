...
- (x) 20.9 Numbers are randomly generated and passed to a method. Write a program to find and maintain the median value as new values are generated.
  ...
  - 也可以通过平衡树来完成，查询操作是O(1），更新操作是O(lgn)
...
优化点是 `c2r(pow-1, pow/10)` 这项其实可以预先计算出来，这样可以变为迭代算法
#+BEGIN_SRC C++
/* coding:utf-8
 * Copyright (C) dirlt
 */
...
int main() {
    int ns[] = {279, 379, 579, 2012, 2002, 0};
    for(int i = 0; ns[i]; i++) {
        int n = ns[i];
        printf("c2(%d) = %d\n", n, c2(n));
    }
...
Python版本思路大致就是，先把item放进去，然后再做balance.
