...
UPDATE@2015-09-08: 这个问题是 [[https://en.wikipedia.org/wiki/Monty_Hall_problem][Monty Hall Problem]], 可以通过 [[file:~/repo/dirtysalt.github.io/codes/py/scripts/monty_hall.py][模拟]] 来计算结果
...
现实中存在下面一些问题：
   - 如何bootstrap呢?也就是设定初始赔率。我们可以首先计算出双方获胜概率p,计算出赔率1/p.为了抽水了降低赔率比如1/p*0.9.这样最后概率计算出来就会是1/0.9了。
...
*** Poor Pigs @ leetcode
leetcode 上面的题目，没有想出来，但是看完 [[https://discuss.leetcode.com/topic/67666/another-explanation-and-solution][解法]](A) 之后觉得非常赞。

一开始我看的是这个 [[https://discuss.leetcode.com/topic/67482/solution-with-detailed-explanation][回答(B)]] , 但是似乎并不完美，尤其是没有解释好多轮的情况。

如果只有一轮情况的话，那么每个dimension的长度就变成了2, 所以答案是 2 ^ pigs. 回答B里面对于一轮情况的解释还是不错的。
