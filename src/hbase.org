...
测试代码可以参考 [[file:../codes/java/asynchbase/src/main/java/com/dirlt/java/asynchbase/AsyncHBase.java][code on github]], 在Get添加Callback之前会检查结果是否已经获取到。如果已经获取的话，那么会在当前的线程就执行callback
