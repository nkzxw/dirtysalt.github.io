...
* 工作经历
** 高级软件架构师, [[http://castbox.fm/][CastBox]], 2016.4 -

基础设施搭建：
1. Python作为主要开发语言，MongoDB作为主要数据库系统。
2. 域名注册，在Cloudflare上管理域名解析。
3. 使用AWS EC2做服务器，ELB做负载均衡，S3做日志和数据存储。

爬虫系统：
1. 数据库中播客数量超过60w，单集数量超过4000w.
2. 播客检查更新延迟可以做到5分钟以内，接近于实时。
3. 对播客和单集的图片根据尺寸进行压缩和裁剪，来优化客户端访问。
